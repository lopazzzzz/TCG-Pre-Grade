...
function drawCardSection(ctx, { label, y, canvas, borders }) {
  ...
  drawCropCell(byKey.tl.canvas, leftX, contentY, 'TOP LEFT');
  drawCropCell(byKey.bl.canvas, leftX, contentY + cornerCellH + gap, 'BOTTOM LEFT');
  drawCropCell(byKey.tr.canvas, rightX, contentY, 'TOP RIGHT');
  drawCropCell(byKey.br.canvas, rightX, contentY + cornerCellH + gap, 'BOTTOM RIGHT');
...
export async function generateReportImage({
  ...
  ctx.fillStyle = COLORS.border;
  ctx.font = 'bold 20px -apple-system, sans-serif';
  ctx.fillText('PRE-GRADE REPORT', WIDTH / 2, 30 + logoH + 34);
  ...
  // ---- Front / Back sections ----
  y = drawCardSection(ctx, { label: 'FRONT', y, canvas: frontCanvas, borders: frontBorders });
  y = drawCardSection(ctx, { label: 'BACK', y, canvas: backCanvas, borders: backBorders });

  // ---- Sub-score row ----
  const subLabels = [
    ['CENTERING', centering.score, `${centering.front_ratio} F · ${centering.back_ratio} B`],
    ['CORNERS', cornersScore, null],
    ['SURFACE', surfaceScore, null],
    ['EDGES', edgesScore, null],
  ];
  const boxGap = 12;
  ...
  companyKeys.forEach((key, i) => {
    ...
    drawStatBox(ctx, x, y, boxW, 84, COMPANY_LABELS[key], Number(c.estimate).toFixed(1), COLORS.text, `${Math.round(c.confidence)}% confidence`);
  });
  y += 84 + 30;
  ...
  if (defectList.length) {
    ...
    ctx.fillText(`FLAWS DETECTED (${defectList.length})`, PAD, y);
    y += 20;
  ...
  ctx.fillStyle = COLORS.muted;
  ctx.font = '11px -apple-system, sans-serif';
  wrapText(ctx, 'AI pre-grade estimate for personal reference only — not affiliated with PSA, CGC, BGS, or TAG, and not a guarantee of actual submission results.', WIDTH / 2 - 300, y, 600, 14);
  y += 34;
  ctx.fillStyle = COLORS.border;
  ctx.font = 'bold 12px -apple-system, sans-serif';
  ctx.fillText(`Generated by Cardify · ${timestamp}`, WIDTH / 2, y);
